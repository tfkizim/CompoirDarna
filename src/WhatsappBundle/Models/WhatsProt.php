...
use \Exception;
use WhatsappBundle\Models\IncompleteMessageException;
use WhatsappBundle\Models\CustomException;
use WhatsappBundle\Models\ProtocolNode;
use WhatsappBundle\Models\BinTreeNodeReader;
use WhatsappBundle\Models\BinTreeNodeWriter;
use WhatsappBundle\Models\Login;
use WhatsappBundle\Models\Logger;
use WhatsappBundle\Models\Constants;
use WhatsappBundle\Helpers\FunctionsHelper;
use WhatsappBundle\Models\rc4;
use WhatsappBundle\Models\WhatsMediaUploader;
use WhatsappBundle\Models\KeyStream;
use WhatsappBundle\Models\TokenMap;
use WhatsappBundle\Events\WhatsApiEventsManager;
use WhatsappBundle\Models\SqliteMessageStore;
use WhatsappBundle\Models\SqliteAxolotlStore;
use WhatsappBundle\Handlers\NotificationHandler;
use WhatsappBundle\Handlers\MessageHandler;
use WhatsappBundle\Handlers\IqHandler;


//if (extension_loaded('curve25519') && extension_loaded('protobuf')) {
    use WhatsappBundle\Models\SenderKeyGroupMessage;
    use WhatsappBundle\Models\SenderKeyGroupData;
    use WhatsappBundle\Models\MediaUrl;
    use WhatsappBundle\Models\ImageMessage;
    use WhatsappBundle\Models\Location;
    use WhatsappBundle\Models\DocumentMessage;
    use WhatsappBundle\Libaxolotl\Util\KeyHelper;
    use WhatsappBundle\Libaxolotl\Ecc\Curve;
    use WhatsappBundle\Libaxolotl\State\PreKeyRecord;
    use WhatsappBundle\Libaxolotl\SessionBuilder;
    use WhatsappBundle\Libaxolotl\SessionCipher;
    use WhatsappBundle\Libaxolotl\Groups\GroupCipher;
    use WhatsappBundle\Libaxolotl\Groups\GroupSessionBuilder;
//}
...
class WhatsProt
{
    ...
    public function __construct($number, $nickname, $debug = false, $log = false, $datafolder = null)
    {
        ...
        $this->functions=new FunctionsHelper();
    ...
    protected function processInboundDataNode(ProtocolNode $node)
    {
        ...
        if ($node->getTag() == 'challenge') {
            ...
            if ($node->hasChild('retry')) {
                $this->sendGetCipherKeysFromUser($this->functions->ExtractNumber($node->getAttribute('from')), true);
            ...
            if ($node->hasChild('error') && $node->getChild('error')->getAttribute('type') == 'enc-v1') {
                $this->v1Only[$this->functions->ExtractNumber($node->getAttribute('from'))] = true;
                $this->messageStore->setPending($node->getAttribute('id'), $node->getAttribute('from'));
...
//          $height = $json->height;
    ...
    protected function sendMessageNode($to, $node, $id = null, $plaintextNode = null)
    {
        ...
        $this->logFile('info', '{type} message with id {id} sent to {to}', ['type' => $type, 'id' => $msgId, 'to' => $this->functions->ExtractNumber($to)]);
        $this->eventManager()->fire('onSendMessage',
            [
                $this->phoneNumber,
                $to,
    ...
    protected function sendSetPicture($jid, $filepath)
    {
        ...
        $data = $this->functions->preprocessProfilePicture($filepath);
        $preview = $this->functions->createIconGD($filepath, 96, true);
